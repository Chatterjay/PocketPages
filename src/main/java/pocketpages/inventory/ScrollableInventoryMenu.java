 ...
 * inventory path while PocketPages supplies the visible extended slots.
 */
public final class ScrollableInventoryMenu extends InventoryMenu {
    ...
    private void replaceVanillaMenuIdentity(int containerId) {
        ...
        access.pocketpages$setMenuType(PocketPages.POCKET_PAGES_MENU.get());
        access.pocketpages$setContainerId(containerId);
    }

    private void clearVanillaSlots() {
        this.slots.clear();
        AbstractContainerMenuAccess access = (AbstractContainerMenuAccess) (Object) this;
        access.pocketpages$getLastSlots().clear();
        access.pocketpages$getRemoteSlots().clear();
    }

    public static void open(Player player) {
        if (player instanceof ServerPlayer serverPlayer) {
            PocketPagesInventoryData.dropLegacyPlaceholderItems(serverPlayer);
            serverPlayer.openMenu(new SimpleMenuProvider((id, inv, p) -> new ScrollableInventoryMenu(id, inv),
                    Component.translatable("container.pocketpages.inventory")));
        }
    }
    ...
    public boolean unlockOne(Player player) {
        if (PocketPagesInventoryData.unlockOne(player)) {
            data.set(0, PocketPagesInventoryData.getUnlocked(player));
            data.set(1, PocketPagesInventoryData.nextUnlockCost(player));
            return true;
        }
        return false;
    }
    ...
    /** Returns the stable PocketPages storage index for a visible grid slot. */
    ...
    public void removed(Player player) {
        if (player instanceof ServerPlayer serverPlayer) {
            ...
            PocketPagesInventoryData.dropLockedItems(serverPlayer);
        } else {
            resultSlots.clearContent();
        }
    }
    ...
    public boolean clickMenuButton(Player player, int id) {
        if (id == 0) {
            if (!PocketPagesInventoryData.canAffordNextUnlock(player)) {
                return false;
            }
            int alreadyUnlocked = PocketPagesInventoryData.getUnlocked(player);
            if (unlockOne(player)) {
                PocketPagesInventoryData.chargeForUnlock(player, alreadyUnlocked);
                return true;
            }
            return false;
        }
    ...
    private static final class SimpleData implements ContainerData {
        ...
        private SimpleData(Player player) {
            this.player = player;
            this.unlockedSlots = PocketPagesInventoryData.getUnlocked(player);
            this.nextUnlockCost = PocketPagesInventoryData.nextUnlockCost(player);
        }

        @Override
        public int get(int index) {
            if (!player.level().isClientSide) {
                unlockedSlots = PocketPagesInventoryData.getUnlocked(player);
                nextUnlockCost = PocketPagesInventoryData.nextUnlockCost(player);
            }
            return switch (index) {
